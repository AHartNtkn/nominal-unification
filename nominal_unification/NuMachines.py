...
class NuMachine():
    ...
    def step(self, nuEq):
        """ Given a nu equation, this will generate the appropriate
            ...
            If it gets an NN equation, it checks that the names match.
            If they don't the constraint is unsatisfyable, otherwise
            the equation is discharged without changing anything
            (since nothing has to be substituted to satisfy the 
            constraint.)
            
            If it gets an NV equation, it must find some name which
            is equal to the name in the first closure while not
            being captured in the scope of the second closure. It
            searches for such a name. If the name in the first closure
            is bound its scope, an appropriate bound variable at the
            same index in the second scope is selected. If the name in
            the first scope isn't bound, a new free variable with the 
            same name is selected.
        
            In the original white paper, this machine is denoted by the
            judgement σ0 ⊦ ν ⇒ν σ1, where σ0 is an input substitution, ν is 
            an input nu problem, and σ1 is an ouput substitution.
    ...
    def eval(self, nuProb):
        """ Given a nu problem, it goes through each in sequence, making
            appripriate additions to the substitution its building.
